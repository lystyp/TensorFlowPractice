...
if SAVE_FLAG == 1 :
    ...
    with tf.Session() as sess2:
        ...
        # init_op = tf.global_variables_initializer() # 初始化依定要在變數定義完才定義，不然初始化完定義的變數都不會初始化了
        # sess2.run(init_op)
        ...
        # saved_result= tf.Variable(data, name="saved_result") # 為何不能放這裡? 因為讀完graph之後就有一個叫saved_result的tensor了，所以放這裡的話會被取成saved_result1(就是後面+1)
        ...
        # # 初始化
        # init_op = tf.global_variables_initializer() # 初始化一定要在變數定義完才定義，不然初始化完定義的變數都不會初始化了
        # sess2.run(init_op)
